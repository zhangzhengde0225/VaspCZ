### 如果对您有帮助，请引用：[VaspCZ：一个提高效率的VASP计算辅助程序](http://www.j.sinap.ac.cn/hjs/CN/10.11889/j.0253-3219.2020.hjs.43.030501)
...
```
---
此处只列出各模块功能，详细接口说明见[API文档](https://github.com/zhangzhengde0225/VaspCZ/blob/master/docs/VaspCZ_python_API.md)
### (1) shell模块
标签|代码|功能
:---:|:---:|:---:
...
2.3|VaspCZ.zzdlib.File.getLine(data,keywords)|给出关键词，搜索有关键词的第一行并返回，返回为字符串和所在的行索引。该功能用于获取特定想信息或者用于判断。
2.4|VaspCZ.zzdlib.File.getAllline(data, keywords)|给出关键词，返回所有带有关键词的所有行，返回为列表。该功能用于选择性获得文件特定行。
2.5|VaspCZ.zzdlib.File.getNullline(data)|获取文件数据中是空位的索引。
2.6|VaspCZ.zzdlib.File.Vaspsh_path()|获取VaspCZ软件默认的PBS提交任务脚本Vasp.sh所在的文件路径。
2.7|VaspCZ.zzdlib.File.VaspCZ_src_path()|获取VaspCZ安装的源码路径
2.8|VaspCZ.zzdlib.File.VaspCZ_software_path()|获取VaspCZ安装的软件路径
2.9|VaspCZ.zzdlib.File.Vasp_pseudo_path()|获取VaspCZ安装时设置的贋势文件夹路径。
...
3.4|VaspCZ.zzdlib.Vasp.modify_POSCAR_Selective_Dynamics(data, indexes)|根据输入的数据和索引修改POSCAR，添加Selective Dynamics, 索引所在的位置设置为T T T, 其他位置设置为 F F F
3.5|VaspCZ.zzdlib.Vasp.modify_INCAR_for_vibration_analysis()|修改当前目录的INCAR为振动分析的INCAR并保存.
