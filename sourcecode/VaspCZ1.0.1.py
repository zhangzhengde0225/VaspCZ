...
|        (4)NEBKeepInputs                                    |
|        (5)k-point mesh test                                |
|        (6)NEB vibration analysis                           |
|        (7)NEB vibration check and result                   | 
|        (8)vibration analysis for solutes in fcc 108 Fe     |
|        (0)back                                             |
|============================================================|
Input(default=2):  ''')
				if ipt9 == '0':
...
Input:  ''')
					...
					while True:
						ipt96 = input('''
|============================================================|
|                  VASP vibration analysis                   |
|------------------------------------------------------------|
|  This is the vibration analysis tool for calcute the atomic|
| vibration frequency, for initial state, saddle state and f-|
|innal state after NEB calculation finished.                 |
|------------------------------------------------------------|
|  Note: running on NEB directory. The ini ini/Opt fin fin/O-|
|pt and each image directory should be in here.              |
|  Plase input the parameters separated by space:            |
|    nodes ppn include_fin                                   |
|    default: 1 8 False, running Vasp with 1 nodes, 8 ppn and|
|do not run the finnal state vibration analysis.             |
|  Go back while input 0                                     |
|============================================================|
Input:  ''')
						if ipt96 == '0':
							...
							try:
								inputs = ipt96.split()
								code = [sys.executable, f'{VaspCZ_path}/VaspVibAna_forNEB.py']
								param = '--nodes,--ppn,--include_fin'.split(',')
								for i in range(len(inputs)):
									code.append(f'{param[i]}={inputs[i]}')
								print(code)
								subprocess.call(code, shell=False)
								exit()
							except Exception as e:
								print(f'Vasp vibration analysis error: {e}')
								continue
				elif ipt9 == '7':
					ipt97 = input('isprint(default:True):')
					code = [sys.executable, f'{VaspCZ_path}/VaspVibAna_Result.py', '--isprint=True']
					if ipt97 != '':
						code[-1] = f'--isprint={ipt97}'
					print(code)
					subprocess.call(code, shell=False)
					exit()
				elif ipt9 == '8':
					ipt98 = input('Inputs nodes and ppn (defalt: 1 8): ')
					code = [sys.executable, f'{VaspCZ_path}/VaspVibAna_forSb.py', '-nd=1', '-np=8']
					if ipt98 != '':
						nodes, ppn = ipt98.split()
						code[2] = f'-nd={nodes}'
						code[3] = f'-np={ppn}'
					print(code)
					subprocess.call(code, shell=False)
